	...
	public function addSlot() {
		...
		if($_POST["slot_interval"] == 0) {
			...
			for($r=0;$r<count($_POST["slot_time_from_hour"]);$r++) {
				if(strval($_POST["slot_time_from_hour"][$r])!='' && strval($_POST["slot_time_from_minute"][$r])!='' && strval($_POST["slot_time_to_hour"][$r]) != '' && strval($_POST["slot_time_to_minute"][$r]) != '') {
		...
		for($i=0; $i<count($resultDate);$i++) {
			...
			if($holidaysQry==0) {
				for($j=0;$j<count($resultTime);$j++) {
					...
					$checkQry=$wpdb->query($wpdb->prepare("SELECT * FROM ".$wpdb->base_prefix.$blog_prefix."booking_slots WHERE slot_active = 1 AND slot_date = %s AND (slot_time_from = %s OR slot_time_to = %s OR (slot_time_from < %s AND slot_time_to > %s) OR (slot_time_from<%s AND slot_time_to>%s) OR (slot_time_from<%s AND slot_time_to>%s)) AND calendar_id= %d",$resultDate[$i],$resultTime[$j]["time_from"],$resultTime[$j]["time_to"],$resultTime[$j]["time_from"],$resultTime[$j]["time_to"],$resultTime[$j]["time_from"],$resultTime[$j]["time_from"],$resultTime[$j]["time_to"],$resultTime[$j]["time_to"],$_POST["calendar_id"]));
